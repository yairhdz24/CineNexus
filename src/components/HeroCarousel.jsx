...
import { ChevronLeft, ChevronRight, Play, Info, Star, Clock, TrendingUp } from 'lucide-react';
...
export default function HeroCarousel({ movies = [] }) {
    ...
    const slideVariants = {
        ...
        center: {
            ...
            transition: {
                ...
                opacity: { duration: 0.3 }, // Faster opacity
                scale: { duration: 0.3 }
            }
        },
        exit: (direction) => ({
    ...
    return (
        ...
        >
            <AnimatePresence mode="wait" custom={direction}>
                ...
                >
                    ...
                    <div className="absolute inset-0">
                        ...
                        {/* Efecto de partículas flotantes - Reducido para rendimiento */}
                        ...
                        <div className="max-w-3xl space-y-6 z-10">
                            ...
                            >
                                <Link
                                    ...
                                    className="group inline-flex items-center gap-1.5 md:gap-3 px-3 py-2 md:px-5 md:py-3 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-bold md:font-black rounded-full shadow-xl transition-all text-sm md:text-base"
                                >
                                ...
                                <Link
                                    to={`/movie/${currentMovie.imdbID}`}
                                    className="inline-flex items-center gap-1.5 md:gap-3 px-3 py-2 md:px-5 md:py-3 bg-white/10 backdrop-blur-xl hover:bg-white/20 text-white font-bold md:font-black rounded-full border border-white/40 hover:border-white/60 transition-all text-sm md:text-base"
                                >
                        ...
                        {currentMovie.Poster !== 'N/A' && (
                            ...
                            >
                                <motion.div
                                    ...
                                    className="relative"
                                >
                                    <div className="absolute -inset-4 bg-gradient-to-r from-red-600 via-rose-600 to-orange-500 rounded-3xl opacity-60 blur-2xl" />
                                    <img
            ...
            {/* Flechas de navegación con estilo premium - Estáticas */}
            <button
                onClick={goToPrevious}
                className="absolute left-6 top-1/2 -translate-y-1/2 p-4 bg-white/10 hover:bg-white/20 backdrop-blur-xl text-white rounded-full transition-all opacity-0 group-hover:opacity-100 z-20 border border-white/20 shadow-2xl hover:scale-105 active:scale-95"
                aria-label="Previous"
            >
            ...
            <button
                onClick={goToNext}
                className="absolute right-6 top-1/2 -translate-y-1/2 p-4 bg-white/10 hover:bg-white/20 backdrop-blur-xl text-white rounded-full transition-all opacity-0 group-hover:opacity-100 z-20 border border-white/20 shadow-2xl hover:scale-105 active:scale-95"
                aria-label="Next"
            >
                ...
                {movies.map((_, index) => (
                    ...
                    >
                        <div className={`absolute inset-0 rounded-full ${index === currentIndex
                            ? 'bg-gradient-to-r from-red-500 via-rose-500 to-orange-500 shadow-lg'
